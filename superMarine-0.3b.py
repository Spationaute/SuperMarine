...
#--------------- SETTINGS --------------------#
def setupAndRun():
	# Global make the variable accessible from
...
def generate():
	...
	for H in range(0,len(HLAY)):
		for cadran in range(0,nCad):
	 		...
	 		#	elif SHAFT[H]==1 and cadran==0 and IMPELLERCUT[sector,cadran,H]==0:
	 		#		impelPatch.append((scafold if sector<NSEC-1 else scafoldLast) + sector + (H*NSEC*nCad))
